...
import dronecan
from dronecan.introspect import to_yaml
...
parser.add_argument(
    "--target-node-id", default=-1, type=int, help="Target CAN node ID (-1 for auto)"
)
parser.add_argument(
    "--dna",
    action="store_true",
    default=False,
    help="run dynamic node allocation server",
)
...
file_hash = base64.b64encode(struct.pack("<I", zlib.crc32(bytearray(firmware_path, "utf-8"))))[
    :7
].decode("utf-8")
...
file_server = dronecan.app.file_server.FileServer(node, path_map={file_hash: firmware_path})
